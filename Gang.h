...
inline size_t ab_get_num_procs(void)
{
    return sysconf(_SC_NPROCESSORS_ONLN) / SZ;
}
...
public:
    ...
    virtual std::vector<uint64_t> & get_scores() = 0;
    virtual ~Tasks() {}
};
...
struct Range
{
    ...
    Range() {}
    Range(size_t b_, size_t e_) : b(b_), e(e_) {}
};
...
public:
    TasksImpl(const std::vector<Range> & ranges_, size_t chunk_size_, bool worksteal_, F && f_)
            : ranges(ranges_), chunk_size(chunk_size_), worksteal(worksteal_), f(std::forward<F>(f_)), scores(ranges.size())
    {
    }
...
//                        if (progress)
//                        {
//                            bis->progressImpl({1, 0});
//                            if (bis->isQuited() || bis->isCancelled())
//                                return;
//                        }
                    }
                }
...
//            runImpl<false>(id);
    ...
    std::vector<uint64_t> & get_scores() override { return scores; }
...
public:
    ...
    Gang(size_t num_, size_t cpu_num_, bool bind_ = false) : num(num_), cpu_num(cpu_num_), bind(bind_)
    {
        thread_id = 0;
        cpu_id = 0;
        CPU_ZERO(&all);
    ...
    template <class F>
    void submit(size_t end, F && f, size_t granular, bool worksteal)
    {
    ...
    template <class F>
    void submitPerCpu(size_t end, F && f, size_t granular, bool worksteal)
    {
        auto n = std::min((end + granular - 1) / granular, num / cpu_num);
        if (n == 0)
            return;
        if (n % cpu_num) {
            std::cerr << "Cannot use submitPerCpu when gang work num isn't a multiplier of cpu num" << std::endl;
            throw;
        }
//            throw DB::Exception("Cannot use submitPerCpu when gang work num isn't a multiplier of cpu num", DB::ErrorCodes::LOGICAL_ERROR);
        auto pn = end / n;
        std::vector<size_t> sz(n, pn);
        for (auto i = 0u; i < end % n; ++i)
        {
    ...
    void rebalance()
    {
        // TODO rebalance task list
    }

    void print_scores(size_t num_ = 0u)
    {
        auto scores = tasks->get_scores();
        auto n = num_ == 0 ? scores.size() : std::min(num_, scores.size());
        for (auto i = 0u; i < n; ++i)
        {
            printf("id:%d, score = %ld\n", i, scores[i]);
        }
    }

    void print_total(size_t num_ = 0u)
    {
        auto scores = tasks->get_scores();
        auto n = num_ == 0 ? scores.size() : std::min(num_, scores.size());
        double sum = 0;
...
private:
    ...
    void reinit(size_t num_)
    {
        ...
        abb2.total = num_;
        __atomic_store_n(&abb2.awaited, num_, MEMMODEL_RELEASE);
    }
};
...
public:
    ...
    GangUtil(size_t num = ab_get_num_procs(), bool bind_ = false)
            : thread_num(num), bind(bind_), gen(std::random_device{}()), dist(0, num - 1), single_id(dist(gen))
    {
        cpu_num = numa_num_configured_nodes();
    }

    size_t numThreads() { return thread_num; }

    size_t id() { return Gang::thread_id; }

    void sync() { ab_barrier_wait(&g->middle); }

    // NOTE: should be used inside submit*
    // how can we randomize this?
    bool single()
    {
        if (single_id == Gang::thread_id)
        {
            single_id = dist(gen);
            return true;
        }
    ...
    // for debug
    void setNum(size_t num) { g->num = num; }

    template <class F>
    void submit(size_t end, F && f, size_t granular)
    {
        if (!g)
            g = create_gang(thread_num, cpu_num, bind);
        g->submit(end, std::forward<F>(f), granular, worksteal);
    }

    // TODO: verify if it's necessary to supply bis for this code path
    template <class F>
    void submit(size_t end, F && f)
    {
        if (!g)
            g = create_gang(thread_num, cpu_num, bind);
    ...
    template <class F>
    void submitPerCpu(size_t end, F && f)
    {
        if (!g)
            g = create_gang(thread_num, cpu_num, bind);
    ...
    template <class F>
    void groupSubmit(size_t end, F && f)
    {
    ...
    T reduceSerial(T * arr, size_t n, Z zero, F && f)
    {
        T r = zero;
    ...
    template <typename T, typename Z, typename F>
    T reduce(T * arr, size_t n, Z zero, F && f)
    {
    ...
    template <typename T>
    T reduce(T * arr, size_t n)
    {
        return reduce(arr, n, (T)0, std::plus<T>());
    }

    template <typename T, typename U = T>
    void assign(T * arr, size_t n, U u = 0)
    {
        T v = u;
    ...
    T scan(T * out, U * in, size_t n, Z zero)
    {
        return scan(out, in, n, zero, std::plus<T>(), false, false);
    }
    template <typename T, typename Z, typename U>
    T scanI(T * out, U * in, size_t n, Z zero)
    {
        return scan(out, in, n, zero, std::plus<T>(), true, false);
    }
    template <typename T, typename Z, typename U>
    T scanBack(T * out, U * in, size_t n, Z zero)
    {
        return scan(out, in, n, zero, std::plus<T>(), false, true);
    }
    template <typename T, typename Z, typename U>
    T scanIBack(T * out, U * in, size_t n, Z zero)
    {
        return scan(out, in, n, zero, std::plus<T>(), true, true);
    }

    template <typename T>
    size_t countSerial(T * in, size_t n)
    {
        return reduceSerial(in, n, (T)0, [&](T a, T b) { return a + (b == std::numeric_limits<T>::max() ? 0 : 1); });
    }
        ...
        if ((n & 1023) == 0 && (reinterpret_cast<size_t>(&in[0]) & 7) == 0)
        {
            ...
            for (size_t k = 0; k < n / 1024; k++)
            {
                ...
                for (size_t j = 0; j < 128; j++)
                    rr += pi[j];
                r += (rr & 255) + ((rr >> 8) & 255) + ((rr >> 16) & 255) + ((rr >> 24) & 255) + ((rr >> 32) & 255) + ((rr >> 40) & 255)
                     + ((rr >> 48) & 255) + ((rr >> 56) & 255);
            }
        }
    ...
    template <typename T, typename U, typename F>
    size_t pack(T *&& out, U * in, size_t n, F && f)
    {
        ...
        groupSubmit(nb, [&](size_t j) {
            auto i = j * BLOCK_SIZE;
            auto len = (j == nb - 1) ? n - i : BLOCK_SIZE;
            packSerial(out + sums[j], in, i, i + len, std::forward<F>(f)); // in base addr, start, end
        });
        free(sums);
        return r;
    }
    ...
    size_t packIndex(T *&& out, U * in, size_t n)
    {
    ...
    template <typename T>
    size_t filter(T *&& out, T * in, size_t n)
    {
    ...
    template <typename T>
    static bool CAS(T * t, T oldv, T newv)
    {
        if constexpr (sizeof(T) == 1)
        {
            return __sync_bool_compare_and_swap(
                    reinterpret_cast<ab8 *>(t), *reinterpret_cast<ab8 *>(&oldv), *reinterpret_cast<ab8 *>(&newv));
        }
        else if constexpr (sizeof(T) == 4)
        {
            return __sync_bool_compare_and_swap(
                    reinterpret_cast<ab32 *>(t), *reinterpret_cast<ab32 *>(&oldv), *reinterpret_cast<ab32 *>(&newv));
        }
        else if constexpr (sizeof(T) == 8)
        {
            return __sync_bool_compare_and_swap(
                    reinterpret_cast<ab64 *>(t), *reinterpret_cast<ab64 *>(&oldv), *reinterpret_cast<ab64 *>(&newv));
        }
        else
        {
    ...
    // SENTINEL.
    template <typename T>
    void dedup(T * data, size_t * index, size_t n)
    {
        submit(n, [&](size_t i) {
            T key = data[i];
            if (key != std::numeric_limits<T>::max() && index[key] == std::numeric_limits<size_t>::max())
            {
    ...
    template <typename T, typename F>
    static bool writeCond(T * t, T newv, F && f)
    {
        T oldv;
        bool r = 0;
        do
    ...
    template <typename T>
    static bool write(T * a, T b)
    {
        return writeCond(a, b, [&](T, T) { return true; });
    }

    template <typename T>
    static bool writeMin(T * a, T b)
    {
        return writeCond(a, b, std::greater<T>());
    }

    template <typename T>
    static bool writeMax(T * a, T b)
    {
        return writeCond(a, b, std::less<T>());
    }

    template <typename T>
    static void writeAdd(T * a, T b)
    {
        T newv, oldv;
        do
        {
